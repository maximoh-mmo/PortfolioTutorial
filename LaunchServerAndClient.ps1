...
$ServerProc = Start-Process -FilePath "$EngineDir\Engine\Binaries\Win64\UnrealEditor-Cmd.exe" `
    -ArgumentList "`"$ProjectPath`" /Game/DemoLevel.DemoLevel?game=/Script/Onset.OnsetGameModeBase -server -log -PORT=7777" `
    -WindowStyle Normal -PassThru
Write-Host "  Server PID: $($ServerProc.Id)" -ForegroundColor Gray

# 3. Wait for server to initialize
Write-Host "[...] Waiting 5s for server to initialize..." -ForegroundColor Yellow
