...
- [ ] Add SQLite amalgamation to `Source/Onset/ThirdParty/SQLite/` (`sqlite3.h` + `sqlite3.c`)
- [ ] Update `Onset.Build.cs` — add SQLite include path and lib to the DS module
...
- [ ] UOnsetPlayerDataSubsystem serialization methods:
...
- [ ] Add `PlayerPlatformID` (FString) and `PlayerPlatform` (FString) to `AOnsetPlayerState`
- [ ] Include Steamworks SDK headers in GameMode for `ISteamGameServer`
- [ ] In `AOnsetGameModeBase::ValidateAuthTicket`:
...
- [ ] After auth completes in PostLogin:
...
- [ ] Store Steam SDK headers path in `Onset.Build.cs` (or reference existing ThirdParty)
...
- [ ] `Server_SelectCharacter` implementation:
...
- [ ] `Server_CreateCharacter` implementation:
...
- [ ] Save-on-disconnect: `AOnsetPlayerController::EndPlay` / `Logout` triggers save
- [ ] Periodic auto-save: `FTimerHandle` in `UOnsetPlayerDataSubsystem`, 5-min interval, saves all connected players
- [ ] Save on level-up / death placeholder (hooks for future systems; save full state for now)

### Wave 5 — Lobby Map & Character Select UI (Days 5-6, ~1.5d)
**Build the character select experience connecting auth to gameplay.**

- [ ] Create minimal lobby level (`/Game/LobbyMap`):
  - Default map for the DS (set in project settings or launch URL)
  - No NPCs, no combat — just the character select HUD
- [ ] Implement `WBP_CharacterSelect` widget:
  - 3 slot panels showing name, level, status (empty/occupied)
  - Create button for empty slots
  - Select button for occupied slots
  - "Enter World" button (enabled when a slot is selected)
  - Occupied slots show character name + level
...
- [ ] Wire "Enter World": `Server_SelectCharacter(Index)` → server responds → `ClientTravel` or `ServerTravel`
- [ ] DS startup flow:
  - DS launches on LobbyMap
  - Clients connect, auth, receive account data, see character select
  - When client presses "Enter World", server **ServerTravel**s to DemoLevel with all connected players
